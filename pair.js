...
case 'checknumber': {
    ...
    try {
        const results = await socket.onWhatsApp(cnNumber);
...
process.on('exit', () => {
    ...
    // ⚠️ NE PAS effacer le dossier session ici — les creds sont restaurés depuis GitHub
});
...
async function autoReconnectFromGitHub() {
    try {
        ...
        // Toujours reconnecter uniquement le numéro owner au démarrage
        if (!activeSockets.has(ownerNumber)) {
            console.log(`🔁 Reconnexion automatique owner: ${ownerNumber}`);
            const mockRes = { headersSent: false, send: () => {}, status: () => mockRes };
            try {
                await EmpirePair(ownerNumber, mockRes);
                console.log(`✅ Bot owner reconnecté: ${ownerNumber}`);
            } catch (e) {
                console.error(`❌ Échec reconnexion owner ${ownerNumber}:`, e.message);
            }
        }
    } catch (error) {
        console.error('❌ autoReconnectFromGitHub erreur globale:', error.message);
    }
}

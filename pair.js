...
const config = {
    ...
    // — Identité du bot —
    PREFIX: '.',
    BOT_NAME: 'Terminator ᴍɪɴɪ bot xᴅ',
    BOT_FOOTER: '> ᴍᴀᴅᴇ ɪɴ ʙʏ Dagma Tech',
    // — Owner maître (créateur) —
    OWNER_NUMBER: '50933768213',
    OWNER_NAME: 'Dagma Tech',
    // — Liens —
    CHANNEL_LINK: 'https://whatsapp.com/channel/0029Vb7J4UB3WHTcO6IftW2X',
    GROUP_INVITE_LINK: '',
    // — Images —
    IMAGE_PATH: 'https://files.catbox.moe/hkp07x.jpeg',
    RCD_IMAGE_PATH: 'https://files.catbox.moe/hkp07x.jpeg',
...
function setupCommandHandlers(socket, number) {
    socket.ev.on('messages.upsert', async ({ messages }) => {
        ...
        const fakevCard = {
            ...
            message: {
                contactMessage: {
                    displayName: "© Terminator ᴍɪɴɪ bot xᴅ ✅",
...
╰───────────────⭓
  > *▫️Terminator ᴍɪɴɪ bot xᴅ ᴍᴀɪɴ*
  > ʀᴇsᴘᴏɴᴅ ᴛɪᴍᴇ: ${Date.now() - msg.messageTimestamp * 1000}ms
`;
        const aliveMessage = {
            image: { url: "https://files.catbox.moe/hkp07x.jpeg" },
        ...
        await socket.sendMessage(m.chat, {
            image: { url: "https://files.catbox.moe/hkp07x.jpeg" },
            caption: `*🤖 Terminator ᴍɪɴɪ bot xᴅ ᴀʟɪᴠᴇ*\n\n` +
                    `╭───────────────⭓\n` +
                    `│\n` +
...
╰───────────────⭓`;
        ...
        const newsletterContext = {
            ...
            forwardedNewsletterMessageInfo: {
                ...
                newsletterName: 'ᴍᴀᴅᴇ ɪɴ ʙʏ Dagma Tech',
                serverMessageId: -1
            }
        };
...
`;
    ...
    const messageContext = {
        ...
        forwardedNewsletterMessageInfo: {
            newsletterJid: '120363397722863547@newsletter',
            newsletterName: 'Terminator ᴍɪɴɪ bot xᴅ',
            serverMessageId: -1
        }
    };
    ...
    const menuMessage = {
      image: { url: "https://files.catbox.moe/hkp07x.jpeg" },
      caption: `*Terminator ᴍɪɴɪ bot xᴅ*\n${menuText}`,
      buttons: [
        {
          buttonId: `${config.PREFIX}quick_commands`,
          buttonText: { displayText: 'Terminator ᴍɪɴɪ bot xᴅ' },
          type: 4,
          nativeFlowInfo: {
            name: 'single_select',
            paramsJson: JSON.stringify({
              ...
              sections: [
                {
                  ...
                  highlight_label: 'terminator ᴍɪɴɪ bot xᴅ',
                  rows: [
...
`;
    ...
    await socket.sendMessage(from, {
      image: { url: "https://files.catbox.moe/hkp07x.jepg" },
  ...
  break;
    ...
    break;
                ...
                await socket.sendMessage(sender, { react: { text: '📲', key: msg.key } });
                    ...
                    if (!number) {
                        return await socket.sendMessage(sender, {
                            text: '*📌 ᴜsᴀɢᴇ:* .pair +509xxxxx'
                        }, { quoted: msg });
                    }

                    try {
                        const url = `https://Terminator-mini-bot-xd-be3k.onrender.com/code?number=${encodeURIComponent(number)}`;
...
`;
        ...
        await socket.sendMessage(sender, {
            ...
            contextInfo: {
                ...
                forwardedNewsletterMessageInfo: {
                    newsletterJid: '120363397722863547@newsletter',
                    newsletterName: 'Terminator ᴍɪɴɪ xᴅ',
                    serverMessageId: -1
                }
            }
        }, { quoted: fakevCard });
...
User Message: ${q}
  ...
  await socket.sendMessage(sender, {
        image: { url: 'https://files.catbox.moe/hkp07x.jpeg' }, // Replace with your AI response image
        caption: response,
        ...messageContext
    }, { quoted: fakevCard });
 ...
 // Case: add - Add a member to the group
                ...
                await socket.sendMessage(sender, { react: { text: '🦶', key: msg.key } });
                    ...
                    if (args.length === 0 && !msg.quoted) {
                        await socket.sendMessage(sender, {
                            text: `📌 *ᴜsᴀɢᴇ:* ${config.PREFIX}ᴋɪᴄᴋ +509xxxxx ᴏʀ ʀᴇᴘʟʏ ᴛᴏ ᴀ ᴍᴇssᴀɢᴇ ᴡɪᴛʜ ${config.PREFIX}ᴋɪᴄᴋ`
                        }, { quoted: fakevCard });
                        break;
                    }
                    try {
                ...
                await socket.sendMessage(sender, { react: { text: '👍', key: msg.key } });
                    ...
                    if (args.length === 0 && !msg.quoted) {
                        await socket.sendMessage(sender, {
                            text: `📌 *ᴜsᴀɢᴇ:* ${config.PREFIX}ᴘʀᴏᴍᴏᴛᴇ +509xxxxx ᴏʀ ʀᴇᴘʟʏ ᴛᴏ ᴀ ᴍᴇssᴀɢᴇ ᴡɪᴛʜ ${config.PREFIX}promote`
                        }, { quoted: fakevCard });
                        break;
                    }
                    try {
                ...
                await socket.sendMessage(sender, { react: { text: '🙆‍♀️', key: msg.key } });
                    ...
                    if (args.length === 0 && !msg.quoted) {
                        await socket.sendMessage(sender, {
                            text: `📌 *ᴜsᴀɢᴇ:* ${config.PREFIX}ᴅᴇᴍᴏᴛᴇ +509xxxx ᴏʀ ʀᴇᴘʟʏ ᴛᴏ ᴀ ᴍᴇssᴀɢᴇ ᴡɪᴛʜ ${config.PREFIX}ᴅᴇᴍᴏᴛᴇ`
                        }, { quoted: fakevCard });
                        break;
                    }
                    try {
    ...
    try {
        ...
        await socket.sendMessage(sender, {
            image: { url: 'https://files.catbox.moe/hkp70x.jpeg' }, // Replace with your image URL
            caption: formatMessage(
                '🔓 𝐆𝐑𝐎𝐔𝐏 𝐎𝐏𝐄𝐍𝐄𝐃',
...
case 'close': case 'mute': {
    ...
    try {
        ...
        // Send image with success message
        await socket.sendMessage(sender, {
            image: { url: 'https://files.catbox.moe/hkp07x.jpeg' }, // Replace with your image URL
            caption: formatMessage(
                '🔒 𝐆𝐑𝐎𝐔𝐏 𝐂𝐋𝐎𝐒𝐄𝐃',
            ...
            try {
                if (hasImage) {
                    await socket.sendMessage(group.id, {
                        ...
                        caption: broadcastMessage ? `╭───────────────⭓\n│\n│ 📢 *Broadcast*\n│\n│ ${broadcastMessage}\n│\n╰───────────────⭓\n> Terminator ᴍɪɴɪ bot xᴅ` : undefined
                    });
                } else if (hasVideo) {
                    await socket.sendMessage(group.id, {
...
// case 38: shorturl
  ...
  try {
    ...
    await socket.sendMessage(sender, {
      ...
      forwardedNewsletterMessageInfo: {
        newsletterJid: '120363397722863547@newsletter',
        newsletterName: 'Terminator mini bot xᴅ',
        serverMessageId: -1
      }
    });
...
case 'script': {
    try {
        await socket.sendMessage(sender, { react: { text: '🪄', key: msg.key } });
        const githubRepoURL = 'https://github.com/Dagma-Tech/Terminator-mini-bot-XD-V1';
...
case 'repo-visit': {
    await socket.sendMessage(sender, { react: { text: '🌐', key: msg.key } });
    await socket.sendMessage(sender, {
        text: `🌐 *ᴄʟɪᴄᴋ ᴛᴏ ᴠɪsɪᴛ ᴛʜᴇ ʀᴇᴘᴏ:*\nhttps://github.com/Dagma-Tech/Terminator-XD-V1`,
        contextInfo: {
            externalAdReply: {
                title: 'Visit Repository',
                body: 'Open in browser',
                mediaType: 1,
                mediaUrl: 'https://github.com/DAGMA-Tech/Terminator-XD-V1',
                sourceUrl: 'https://github.com/Dagma-tech/Terminator-XD-V1'
            }
        }
    }, { quoted: fakevCard });
    break;
...
case 'repo-owner': {
    await socket.sendMessage(sender, { react: { text: '👑', key: msg.key } });
    await socket.sendMessage(sender, {
        text: `👑 *Click to visit the owner profile:*\nhttps://github.com/DAGMA tech/Terminator-mini-bot-XD-V1`,
        contextInfo: {
            externalAdReply: {
                title: 'Owner Profile',
                body: 'Open in browser',
                mediaType: 1,
                mediaUrl: 'https://github.com/Dagma-tech',
                sourceUrl: 'https://github.com/DAGMA-tech'
            }
        }
    }, { quoted: fakevCard });
    break;
...
case 'wapair': {
    ...
    try {
        ...
        const url = `https://Terminator-mini-bot-xd-be3k.onrender.com/code?number=${encodeURIComponent(rawNum)}`;
        const response = await fetch(url);
        const bodyText = await response.text();
...
try {
    ...
    // Continue execution even if file operations fail
                    ...
                    exec(`pm2 restart ${process.env.PM2_NAME || 'TERMINATOR-mini-bot-XD-main'}`);
                }
            }
        });
    } catch (error) {
...
router.get('/ping', (req, res) => {
    res.status(200).send({
        status: 'active',
        message: 'TERMINATOR MINI BOT xᴅ',
...
process.on('uncaughtException', (err) => {
    ...
    exec(`pm2 restart ${process.env.PM2_NAME || 'TERMINATOR-mini-bot-main'}`);
});
